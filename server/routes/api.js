...
const { getProblemNum } = require('../config/problemDetails.js');
...
router.post('/workspace', async (req, res) => {
  ...
  const obj = new Workspace({ problem, id: getProblemNum(problem.link) });
  await obj.save();
  return res.json(obj);
});
...
router.get('/workspace/:id', async (req, res) => {
  if (typeof req.params.id !== 'string'){
    ...
    Workspace.findOne({id: req.params.id}, function(err, workspace){
      if(err){
